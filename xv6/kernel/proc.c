...
found:
  ...
  p->completion_tick = -1;
  p->waiting_ticks = 0;
  printk("TRACE arrival tick=%d pid=%d\n", ticks, p->pid);
...
kfork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  np->state = RUNNABLE;
  #ifdef MLFQ
  enqueue(0,np);  //pushed to queue (new process);
  #endif
  release(&np->lock);
...
kexit(int status)
{
  ...
  printk("METRIC pid=%d arrival=%d first=%d completion=%d waiting=%d turnaround=%d response=%d\n",
       p->pid,
       p->arrival_tick,
       p->first_run_tick,
       p->completion_tick,
       p->waiting_ticks,
       p->completion_tick - p->arrival_tick,
       p->first_run_tick - p->arrival_tick);
  p->state = ZOMBIE;
...
struct proc *deque(int q){
  ...
  if(queues[q].head==0) queues[q].tail=0;
  p->next=0;
  release(&queues[q].lock);
  return p;
}
struct proc *deque_highest_priority_p(){
   struct proc *p;
   for(int i=0;i<4;i++){
    p=deque(i);
    if(p!=0){
...
int mlfq_tick(struct proc *p){
  p->ticks_used++;
  if(p->ticks_used>=time_slice[p->priority]){
    return 1;
  }
  return 0;
}
...
void mlfq_requeue(struct proc *p){
  enqueue(p->priority, p);
}
int mlfq_highest_priority_ready(struct proc *p){
  for(int q=0;q<p->priority;q++){
    acquire(&queues[q].lock);
    
    int ready=(queues[q].head!=0);
    release(&queues[q].lock);
    if(ready) return 1;
  }
  return 0;
...
void update_waiting_ticks(void){
  struct proc *p;
  ...
  c->proc = 0;
   ...
   for(;;){
    ...
    if(p->state==RUNNABLE){
      ...
      c->proc=p;
      printk("TRACE dispatch tick=%d pid=%d\n", ticks, p->pid);
...
procdump(void)
{
  ...
  for (p = proc; p < &proc[NPROC]; p++) {
    ...
    if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
    printk("%d %s %s Q%d ticks=%d rtime=%d", p->pid, state, p->name,p->priority,p->ticks_used,p->rtime);
    printk("\n");
  }
}
